...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        if [[ ! -f "/etc/x-ui/x-ui.db" ]]; then
            ...
            /usr/local/x-ui/x-ui.sh setting -username ${usernameTemp} -password ${passwordTemp}
            /usr/local/x-ui/x-ui.sh setting -port ${portTemp}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/mRcongzi/x-ui/archive/refs/tags/${last_version}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/mRcongzi/x-ui/archive/refs/tags/${last_version}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/mRcongzi/x-ui/main/x-ui.sh
